...
func (p *Provider) LendMarkets(ctx context.Context, asset id.Asset) ([]model.LendMarket, error) {
	...
	filtered := defillama.FilterPools(pools, "mantle", "lendle")
	...
	if len(out) == 0 {
		return nil, clierr.New(clierr.CodeUnavailable, "lendle markets unavailable")
	}
	return out, nil
}
...
func (p *Provider) LendRates(ctx context.Context, asset id.Asset) ([]model.LendRate, error) {
	...
	if len(out) == 0 {
		return nil, clierr.New(clierr.CodeUnavailable, "lendle rates unavailable")
	}
	return out, nil
}
